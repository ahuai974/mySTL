...
{	
	//获取地址
	template<class Tp>
	constexpr Tp* addrees_of(Tp& value) noexcept
	{
		return &value;
	}
// 获取 / 释放 临时缓冲区
	...
	pair<T*, ptrdiff_t> get_buffer_helper(ptrdiff_t len, T*)
	{
		if (len > static_cast<ptrdiff_t>(INT_MAX / sizeof(T)))
			len = INT_MAN / sizeof(T);
		//不断申请内存，失败则申请空间减半，直至成功
		while (len > 0)
		{	
	...
	template<class ForwafdIterator,class T>
	class temporary_buffer
	{
	private:
		ptrdiff_t original_len;//申请缓冲区的大小
